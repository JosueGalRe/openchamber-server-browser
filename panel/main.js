...
${tab.url}`;
  ...
  var reportedRatio = null;
  var syncPixelRatio = () => {
    const ratio = window.devicePixelRatio || 1;
    if (ratio === reportedRatio) return;
    reportedRatio = ratio;
    void host.serviceRequest({ method: "POST", path: "/browser/viewer", body: JSON.stringify({ devicePixelRatio: ratio }) }).then((result) => {
      if (result.status !== 200) reportedRatio = null;
    }, () => {
      reportedRatio = null;
    });
  };
