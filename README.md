...
- An OpenChamber build that includes [commit `0db5ed016`](https://github.com/openchamber/openchamber/commit/0db5ed016215466c1007d946a14c6a5e5b8badde), which adds tab targets, viewer identity, and dock resizing. No released version includes it yet. 0.7.0 does not work on 2.0.0: agents there cannot name a tab, so a page they open in a background tab would be out of their reach. The manifest's `openchamber.engines.openchamber` floor, still `>=2.0.0` on this branch, moves to that release when it ships.
...
Updating from 0.6.x: `browser.open` without a tab id now opens the page in a background tab, unless the visible tab is still blank, so the page you are watching is never replaced. The dock lists the new page as a tab; select it to watch. The agent receives its id and names it in later actions.
...
0.7.0 builds against the SDK at upstream commit `0db5ed016`, packed in `vendor/` until a registry release includes it. Only a Linux web host built from that commit has been exercised.
...
| Recover tab management and browser controls | Viewer-side tabs, popups, and controls are ported, and agents name tabs by id. Agents cannot close tabs or bring one to the viewer. | Validate a multi-tab agent workflow on a released host. |
| Harden scoped viewing | Browser state is isolated per project/chat, and dock calls and input carry the host's viewer identity and frame sequence, so stale input cannot cross a scope or tab switch. The visible selection is still shared by every viewer. | Validate concurrent viewers on a released host. |
...
This assessment is based on the [service contracts at commit `0db5ed016`](https://github.com/openchamber/openchamber/blob/0db5ed016215466c1007d946a14c6a5e5b8badde/packages/sdk/GUEST_SERVICES.md), which add tab targets, viewer identity, and dock resizing to those of SDK 2.0.0. It records what remains after adopting them. These are discussion points for upstream, not agreed API changes.
...
None for the workflows mapped so far. Viewer identity and stale-input fencing, the last one, arrived in `0db5ed016`.
...
| --- | --- | --- |
| Supported runtime behavior | 0.7.0 builds against an unreleased SDK commit, and only a Linux web host built from it has been exercised. | A reproducible install against a released host/SDK combination, followed by validation on every runtime and transport we advertise. |
